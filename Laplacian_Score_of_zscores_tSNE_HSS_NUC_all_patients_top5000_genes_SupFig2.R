...
#ggsave(paste("/Users/baizilong/Documents/Dana-dataset/RA_paper/summary_scores_vs_average_cell_density_SupFig2",".png",sep=""))
#dev.off()
######
...
sp_ls.tsne + 
  ...
  scale_y_continuous(limits=c(1500, 7000)) #name="Cell Density", 
#ggsave(paste("/Users/baizilong/Documents/Dana-dataset/RA_paper/summary_scores_vs_average_cell_density_5000_genes_SupFig2",".png",sep=""))
#dev.off()
...
# kendall.result <- cor.test(df_sampled.tsne$x, df_sampled.tsne$y, method="kendall")
# 
# df_sampled.tsne <- data.frame(x = sign(as.numeric(kendall.result$estimate[1]))*gex_zscores_sampled.tsne$Y, #sign(as.numeric(kendall.result$estimate[1]))*
#                               y = nuc_density_scores$nuc_density_scores,
#                               nds = nuc_density_scores$nuc_density_scores)
# 
# df_sampled.tsne.summary_score <- data.frame(gene_summary_score = sign(as.numeric(kendall.result$estimate[1]))*gex_zscores_sampled.tsne$Y, #sign(as.numeric(kendall.result$estimate[1]))*
#                                             nuc_density_scores = nuc_density_scores$nuc_density_scores,
#                                             row.names = nuc_density_scores$X)
# 
# write.csv(df_sampled.tsne.summary_score,paste('/Users/baizilong/Documents/Dana-dataset/Exploration-results/Paper_Gene-Expression_Low-Inflammatory-Synovium_Pain/pain_related_top',s,'_gene_summary_scores_nuc_benchmarking_all_patients_all_genes.csv',sep=''))
# 
# kendall.result <- cor.test(df_sampled.tsne$x, df_sampled.tsne$y, method="kendall")
# pearson.result <- cor.test(df_sampled.tsne$x, df_sampled.tsne$y, method="pearson")
# 
...
# 
# sp_ls.tsne<-ggplot(df_sampled.tsne, aes(x, y, color=nds, label=nds)) +
#   geom_point(aes(size=10), show.legend=FALSE) + 
#   geom_smooth(method = "lm", se = FALSE) +
#   theme(text = element_text(size = 20)) +
#   ggtitle(figure_title)+
#   scale_color_gradient(low="red", high="blue") +
#   scale_x_continuous(name="Gene Subset Expression Summary Score") + 
#   scale_y_continuous(name="Average Nuclei Density", limits=c(1500, 7000))
# ggsave(paste("/Users/baizilong/Documents/Dana-dataset/Exploration-results/Paper_Gene-Expression_Low-Inflammatory-Synovium_Pain/Plots_for_paper/summary_scores_vs_average_nuclei_density_all_patients_all_genes",".png",sep=""))
# ######
# 
# 

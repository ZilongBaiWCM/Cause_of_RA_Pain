...
#pain_exp_data <- exp_data[toupper(exp_data$SYMBOL.1) %in% toupper(pain_genes$Symbol), ]
pain_exp_data <- exp_data[toupper(row.names(exp_data)) %in% toupper(row.names(pain_genes)),]
...
for (r in row.names(vas_scores)){
  ...
  if (vas_scores[r, 'Pathotype'] == 'myeloid'){
    vas_scores[r, 'Inflammation.Level'] <- 'Mixed: myeloid'
  }
...
coloring <- c('purple', 'green', 'orange')
...
for(infla in unique_inflammation){
  ...
  #ggsave(plot = sp_ls.tsne, paste("/Users/baizilong/Documents/Dana-dataset/RA_paper/peac_validation_pain_genes_summary_scores_vs_VAS_",infla,"_Fig2.png",sep=""))
